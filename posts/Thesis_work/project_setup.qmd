...
We decided in the end to keep it simple, and use only the Multi-state Bar Exam (MBE) owing to its multiple choice, single answer format. This would give us a definitive evaluation strategy for our LLM that is already a well accepted paradigm ( used similarly in the [MMLU (Mulit-task Language Understanding)](https://huggingface.co/datasets/cais/mmlu) benchmark). Our eval technique basically boiled down to comparing the model's selected choice with the actual correct option. 
